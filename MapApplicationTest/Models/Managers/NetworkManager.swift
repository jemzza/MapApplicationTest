...
//    guard Reachabilty.HasConnection() else { return }
  ...
  private func orderDictionaryFrom(_ order: Order) -> NSDictionary {
    return NSDictionary(objects: [order.id, order.ownerId, order.location!, order.latitude, order.longitude, order.gender!, order.age!, order.weight!, order.interests!, order.date, order.dateOfEnd], forKeys: [keyObjectId as NSCopying, keyOwnerId as NSCopying,keyLocation as NSCopying, keyLatitude as NSCopying, keyLongitude as NSCopying, keyGender as NSCopying, keyAge as NSCopying, keyWeight as NSCopying, keyInterests as NSCopying, keyDate as NSCopying, keyDateOfEnd as NSCopying])
  }
